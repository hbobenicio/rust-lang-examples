...
impl App {
    ...
    fn connect_signals(&self) {
      self.main_window.connect_delete_event(|_win, _event: &gdk::Event| {
          gtk::main_quit();
          Inhibit(false)
      });
      self.add_toolbutton.connect_clicked(|_b| {
          println!("Add button clicked");
      });
      self.remove_toolbutton.connect_clicked(|_b| {
          println!("Remove button clicked");
      });
      self.start_button.connect_clicked(|_b| {
